...
class Form {
	...
	/**
	 * Form constructor.
	 * @param $registry \Framework\Registry
	 * @param $action
	 * @param $method
//	 * @param Controls $__controls
	 */
	public function __construct(\Framework\Registry $registry, $action = '', $method = self::METHOD_POST)
	{
...
//		$this->__is_autocomplete = $__is_autocomplete;
		...
		$this->controller = new \Framework\AbstractController($registry);
	}
	...
	public function addInput($type, $name, $classes = '', $is_error = false, $error_text = '')
	{
		...
		$id = "input-{$name}";
		$label = $this->controller->getLanguage()->get("entry_{$name}");
	...
	{
		return "<form id='{$this->getId()}' action='{$this->getAction()}' class='' method='{$this->getMethod()}' enctype='multipart/form-data'>";
	}
